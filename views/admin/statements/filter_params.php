...
<div class="_filters _hidden_inputs">
    <?php
    foreach($statements_sale_agents as $agent){
        echo form_hidden('sale_agent_'.$agent['sale_agent']);
    }
    foreach($statements_statuses as $_status){
        $val = '';
        if($_status == $this->input->get('status')){
            $val = $_status;
        }
        echo form_hidden('statements_'.$_status,$val);
    }
    foreach($statements_years as $year){
        echo form_hidden('year_'.$year['year'],$year['year']);
    }
    foreach($payment_modes as $mode){
        echo form_hidden('statement_payments_by_'.$mode['id']);
    }
