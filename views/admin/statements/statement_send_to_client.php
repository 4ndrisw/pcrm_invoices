...
<div class="modal fade email-template" data-editor-id=".<?php echo 'tinymce-' . $statement->id; ?>" id="statement_send_to_client_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php echo form_open('admin/statements/send_to_email/' . $statement->id); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">
                    <?php echo _l('statement_send_to_client_modal_heading'); ?>
                </h4>
            </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            $selected = [];
                            $contacts = $this->clients_model->get_contacts($statement->clientid, ['active' => 1, 'statement_emails' => 1]);
                            foreach ($contacts as $contact) {
                                array_push($selected, $contact['id']);
                            }
                            if (count($selected) == 0) {
                                echo '<p class="text-danger">' . _l('sending_email_contact_permissions_warning', _l('customer_permission_statement')) . '</p><hr />';
                            }
                            echo render_select('sent_to[]', $contacts, ['id', 'email', 'firstname,lastname'], 'statement_estimate_sent_to_email', $selected, ['multiple' => true], [], '', '', false);
                            ?>
                        </div>
                        ...
                        <div class="checkbox checkbox-primary">
                            ...
                            <label for="attach_pdf"><?php echo _l('statement_send_to_client_attach_pdf'); ?></label>
                        </div>
                        <div class="checkbox checkbox-primary">
                            <input type="checkbox" name="attach_statement" id="attach_statement">
                            <label for="attach_statement"><?php echo _l('attach_statement'); ?>
                            <a href="<?php echo admin_url('clients/client/' . $statement->clientid . '?group=statement'); ?>" target="_blank">
                    ...
                    <hr />
                    <h5 class="bold"><?php echo _l('statement_send_to_client_preview_template'); ?></h5>
                    <hr />
                    <?php echo render_textarea('email_template_custom', '', $template->message, [], [], '', 'tinymce-' . $statement->id); ?>
...
<script>
    $(function(){
        ...
        $('#statement_send_to_client_modal form').submit(function(){
