...
<div id="stats-top" class="hide">
    ...
    <?php
      ...
      if (!$has_permission_view) {
          $where_all .= get_statements_where_sql_for_staff(get_staff_user_id());
      }
      $where_all = trim($where_all);
      if (endsWith($where_all, ' AND')) {
          $where_all = substr_replace($where_all, '', -4);
      }
      $total_statements = total_rows(db_prefix() . 'statements', $where_all);
      ?>
    <div class="quick-top-stats">
        <dl
          ...
          if ($status == Statements_model::STATUS_CANCELLED) {
              continue;
          }
          ...
          $total_by_status = total_rows(db_prefix() . 'statements', $where);
          $percent         = ($total_statements > 0 ? number_format(($total_by_status * 100) / $total_statements, 2) : 0); ?>

            <div class="tw-px-3 tw-py-4 sm:tw-p-4">
                <dt class="tw-font-medium text-<?php echo get_statement_status_label($status); ?>">
                    <?php echo format_statement_status($status, '', false); ?>
                </dt>
                <dd class="tw-mt-1 tw-flex tw-items-baseline tw-justify-between md:tw-block lg:tw-flex">
                    <div class="tw-flex tw-items-baseline tw-text-base tw-font-semibold tw-text-primary-600">
                        <?php echo $total_by_status; ?> / <?php echo $total_statements; ?>
                        <span class="tw-ml-2 tw-text-sm tw-font-medium tw-text-neutral-500">
                            <a href="#" data-cview="statements_<?php echo $status; ?>"
                                onclick="dt_custom_view('statements_<?php echo $status; ?>','.table-statements','statements_<?php echo $status; ?>',true); return false;">
                                <?php echo _l('view'); ?>
                            </a>
                        </span>
                    </div>
